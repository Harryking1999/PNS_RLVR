...
| **Merged models 统一目录** | `/home/fuzhizhang.fzz/model/merge_models/` (从 `/ssdwork/fuzhizhang/merged_models/` 迁移) |
| DAPO-PNS merged ckpts | `merge_models/DAPO-PNS-step{200,400,600,737,800,850}` |
| DAPO-PNS dry-run merged ckpts | `merge_models/DAPO-PNS-dryrun-step{200,400,600,700}` |
| DAPO baseline merged ckpts | `merge_models/DAPO-step{200,...,2550}` |
| Forking / Step-Forking ckpts | `merge_models/{Forking,Step-Forking}-DAPO-step*` |
| **原始 ckpt 统一目录** | `/home/fuzhizhang.fzz/model/ckpts/DAPO/` (从 `/ssdwork/fuzhizhang/ckpts/` 迁移) |
| DAPO-PNS 原始 ckpt (含optimizer) | `ckpts/DAPO/RLVR-Qwen3-1.7B-DAPO-PNS/global_step_{200,400,600,737,800,850}` |
| DAPO-PNS dry-run 原始 ckpt | `ckpts/DAPO/RLVR-Qwen3-1.7B-DAPO-PNS-dryrun/global_step_{200,400,600,700}` |
...
| **PNS 分析结果 (统一)** | `/home/fuzhizhang.fzz/model/merge_models/pns_results/` (已合并旧结果) |
...
**输出文件** (默认存到 `/ssdwork/fuzhizhang/pns_results/{model_name}/`，可用 `--output_dir` 指定):
...
| Dry-run PNS 自动流水线 | `scripts/run_dryrun_pns_after_eval.sh` |
...
- **GPU 0**: R1-Distill-PNS-step200 评测 (aime24/aime25/math500/math500_noop)。日志: `nohup.out.eval_r1distill_gpu0`
- **GPU 1**: R1-Distill-PNS-step200 PNS forward (test_all_steps)。日志: `nohup.out.pns_r1distill_step200_fwd`
- **GPU 4-7**: R1-Distill-1.5B DAPO-PNS 训练中（~step248/850）。日志: `nohup.out.0316.DAPO_PNS_distill`。Ckpt 存: `/home/fuzhizhang.fzz/model/ckpts/DAPO/RLVR-R1-Distill-1.5B-DAPO-PNS/`

### 待办事项 (To-Do)

1. R1-Distill 后续 ckpt (step400, step600...) 出来后继续 merge + 评测 + PNS
2. R1-Distill-PNS-step200 eval 完成后跑 reverse PNS
3. 对比 PNS 干预组 vs dry-run 组的训练曲线（wandb 项目: DAPO）

### 已完成与中间结果 (Completed & Results)

#### DAPO-PNS 训练 (干预组) — 已完成 850 steps

Checkpoint 保存于 `ckpts/DAPO/RLVR-Qwen3-1.7B-DAPO-PNS/`，已 merge 到 `merge_models/DAPO-PNS-step*`。

#### DAPO-PNS dry-run 训练 (对照组) — 已完成 700 steps (1 epoch)

Checkpoint: `ckpts/DAPO/RLVR-Qwen3-1.7B-DAPO-PNS-dryrun/global_step_{200,400,600,700}`，已 merge 到 `merge_models/DAPO-PNS-dryrun-step{200,400,600,700}`。

> **为什么预估 850 步但 700 步就跑完？** `total_epochs=1` + `enable_filter_groups=True`。548 步用 1 个 gen batch (64 prompts)，151 步因 filter_groups 重新生成用了 2 个 gen batches (128 prompts)。实际消耗: 548×64 + 151×128 = 54,400 prompts = 整个训练集。

#### DAPO-PNS dry-run 评测结果 (Greedy Acc@1 % / Len@1)

| Step | AIME24 | AIME25 | MATH500 | MATH500-Noop |
|------|--------|--------|---------|--------------|
| 200  | 6.7 / 6502 | 3.3 / 4380 | 64.0 / 1612 | 55.4 / 1951 |
| 400  | 6.7 / 9647 | 6.7 / 5633 | 63.6 / 2535 | 53.4 / 3288 |
| 600  | 10.0 / 6627| 3.3 / 5148 | 64.6 / 2035 | 54.2 / 2907 |
| 700  | 3.3 / 7084 | 6.7 / 5583 | 63.0 / 1610 | 54.4 / 2545 |

**DAPO-PNS dry-run Pass@16 (%) / Len@16**

| Step | AIME24 | AIME25 | MATH500 | MATH500-Noop |
|------|--------|--------|---------|--------------|
| 200  | 20.0 / 1005 | 6.7 / 857  | 83.8 / 567  | 79.8 / 624  |
| 400  | 13.3 / 1948 | 13.3 / 1769| 87.4 / 1201 | 80.0 / 1365 |
| 600  | 16.7 / 3594 | 13.3 / 3885| 85.4 / 1629 | 80.8 / 1907 |
| 700  | 16.7 / 1451 | 13.3 / 1431| 84.8 / 830  | 80.2 / 955  |

> Dry-run MATH500 Acc@1 在 63-65% 波动，与 PNS 干预组相当（64-68%）。但 dry-run 的 MATH500-Noop 更稳定（53-55%），而干预组后期 Noop 严重下降（43-50%）。

#### DAPO-PNS dry-run PNS 离线估算

**正向 PNS (Forward)**

| Model | Solved | Steps | mean_PNS | Ent↔PNS corr | HE_PNS | LE_PNS | Δ(HE-LE) |
|-------|--------|-------|----------|---------------|--------|--------|----------|
| dryrun-step200 | 42 | 1113 | 0.4376 | 0.142 | 0.4339 | 0.4390 | -0.005 |
| dryrun-step400 | 41 | 2147 | 0.2997 | 0.215 | 0.3175 | 0.2929 | +0.025 |
| dryrun-step600 | 38 | 2268 | 0.3886 | 0.131 | 0.3938 | 0.3867 | +0.007 |
| dryrun-step700 | 40 | 1180 | 0.3951 | 0.052 | 0.3833 | 0.3995 | -0.016 |

**反向 PNS (Reverse)**

| Model | Wrong | Steps | mean_reverse_PNS |
|-------|-------|-------|------------------|
| dryrun-step200 | 40 | 1292 | 0.2070 |
| dryrun-step400 | 43 | 3494 | 0.1979 |
| dryrun-step600 | 39 | 6038 | 0.1222 |
| dryrun-step700 | 40 | 1791 | 0.1875 |
...
| 文件 | 内容 |
|------|------|
| `/home/fuzhizhang.fzz/model/merge_models/eval_results.json` | DAPO-PNS step{200-850} 全指标 (迁移自 ssdwork) |
| `/home/fuzhizhang.fzz/model/merge_models/eval_results_baselines.json` | 全部 6 个 baseline 模型 Acc@1 + Pass@16 (迁移自 ssdwork) |
| `/home/fuzhizhang.fzz/model/merge_models/eval_results_dryrun_gpu0.json` | Dry-run step{200,600} 评测结果 (进行中) |
| `/home/fuzhizhang.fzz/model/merge_models/eval_results_dryrun_gpu1.json` | Dry-run step400 评测结果 (进行中) |
