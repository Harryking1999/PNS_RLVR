            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if pns_enable and self._pns_boundary_lookup is not None:
                        with marked_timer("pns_ablation", timing_raw, color="magenta"):
                            actor_cfg = self.config.actor_rollout_ref.actor
                            # PNS reuses entropy_top_ratio for step scoring (which tokens
                            # count as "high entropy" within a step). It may be None when
                            # Step Forking DAPO is disabled; fall back to 0.2.
                            # step_top_ratio is fixed to 1.0: no per-rollout pre-filtering;
                            # the global pns_step_ratio is the sole control for how many
                            # steps are tested.
                            ...
                            def pns_generate_fn(ablation_batch, pns_num_rollouts=5):
                                ...
                                # Pad to dp_size divisor so dispatch across ranks works
                                ...
                                # Unpad: each padded prompt produces pns_num_rollouts responses
