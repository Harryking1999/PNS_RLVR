...
# Adapted from run_dapo_qwen3_14b.sh for Qwen3-1.7B-Base on 4x H100 (80GB) GPUs
...
project_name='DAPO'
exp_name='RLVR-Qwen3-1.7B-entropy-forking-DAPO'
...
max_response_length=$((1024 * 16))   # 16384 (14B脚本用20K, 1.7B适当缩短节省显存; 如需更长可改回 1024*20)
...
# 14B脚本用128卡: train_prompt_bsz=512, 这里按4卡等比缩放
train_prompt_bsz=16
gen_prompt_bsz=$((train_prompt_bsz * 4))  # 64 (n_resp减半后适当增大, 维持GPU利用率)
n_resp_per_prompt=8
...
CKPTS_DIR="${PROJECT_ROOT}/ckpts/${project_name}/${exp_name}"
...
python3 -m recipe.dapo.main_dapo \
    ...
    # 如需启用 entropy top 20% 梯度过滤, 取消下面这行的注释:
